...
## 2026-05-28 - Add ML leaderboard and replay traces

- Changed: added `leaderboard.py`, `replay.py`, `ml_dashboard.py`, `test_leaderboard.py`, `test_replay.py`, and `test_ml_dashboard.py`; wired leaderboard updates into `evaluate_model.py`, `compare_models.py`, `model_report.py`, and `train_pipeline.py`; added `evaluate_model.py --trace-dir` replay-trace output; bumped the dev version to `0.8.5-dev`; refreshed README and parser tests.
- Why: model iteration needed long-term memory. A single report says how one run went, but it did not answer "which model/settings have been best so far?" or let a strange model run be inspected frame by frame after the fact.
- Behavior: evaluation can now save per-game replay JSON with seed, settings, actions, player position, nearby rocks, score/life/combo state, and event frames; saved evaluation/comparison/model-report/pipeline JSON can be appended to a local leaderboard; training pipeline and model reports update `runs/model_leaderboard.json` by default; `ml_dashboard.py` summarizes data quality, objective coverage, variant coverage, leaderboard status, and next-step recommendations.
- Verification: ran `python3 -m unittest`; ran `python3 -X pycache_prefix=/private/tmp/rockfall-pycache -m compileall -q .`; ran `python3 evaluate_model.py --games 1 --max-frames 120 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --report /private/tmp/rockfall-v085-eval.json --leaderboard /private/tmp/rockfall-v085-leaderboard.json --leaderboard-tag eval-smoke --trace-dir /private/tmp/rockfall-v085-replays --trace-stride 20`; ran `python3 replay.py /private/tmp/rockfall-v085-replays/game_model.pkl-game-1-seed-42.json --limit 4`; ran `python3 compare_models.py game_model.pkl --include-rule-baseline --include-ore-hunter-baseline --games 1 --max-frames 120 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --report /private/tmp/rockfall-v085-comparison.json --leaderboard /private/tmp/rockfall-v085-leaderboard.json --leaderboard-tag comparison-smoke`; ran `python3 leaderboard.py --leaderboard /private/tmp/rockfall-v085-leaderboard.json --limit 5`; ran `python3 model_report.py --data game_data.json --profiles standard --games 1 --max-frames 120 --difficulty normal --player-speed 8 --lives 3 --report /private/tmp/rockfall-v085-model-report.json --leaderboard /private/tmp/rockfall-v085-leaderboard.json --leaderboard-tag model-report-smoke`; ran `python3 ml_dashboard.py --data game_data.json --leaderboard /private/tmp/rockfall-v085-leaderboard.json --report /private/tmp/rockfall-v085-dashboard.json --top 4`; ran `python3 collect_policy_data.py --data /private/tmp/rockfall-v085-policy.json --policy ore-hunter --games 1 --max-frames 180 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --report /private/tmp/rockfall-v085-policy-report.json`; ran `python3 train_pipeline.py --data /private/tmp/rockfall-v085-policy.json --candidate /private/tmp/rockfall-v085-candidate.pkl --report /private/tmp/rockfall-v085-pipeline.json --leaderboard /private/tmp/rockfall-v085-leaderboard.json --leaderboard-tag pipeline-smoke --games 1 --max-frames 120 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --estimators 5`; ran JSON validation with `python3 -m json.tool` on the generated eval, comparison, model-report, pipeline, dashboard, and leaderboard artifacts; ran a stale-text scan for old active-version/objective wording outside historical devlog entries; ran `git diff --check`.
- Risks/Notes: the smoke runs intentionally used short frame limits, so their leaderboard scores are only plumbing checks and not meaningful model-quality evidence. The dashboard correctly reports that tracked `game_data.json` is legacy data with no `ore_target_v2` or recorded variant samples; collect fresh v2 data before promoting a real model.

## 2026-05-25 - Add visible model training loop

- Changed: added `train_pipeline.py` and `test_train_pipeline.py`; added the `ore-hunter` built-in policy in `policies.py`; wired ore-hunter into `collect_policy_data.py`, `compare_models.py`, and `model_report.py`; expanded `play_with_model.py` debug lines with prediction confidence, danger pressure, ore pull, and per-rock focus; added an ore caught/missed game-over summary in `game_core.py`; bumped the dev version to `0.8.4-dev`; refreshed README and tests.
- Why: the project needed a clearer machine-learning loop: collect v2 ore-target data, train a candidate, compare it against the tracked model and rule baselines, and make model decisions visible during play.
- Behavior: `python3 train_pipeline.py` now trains `runs/pipeline_candidate.pkl`, compares baseline/candidate/policy baselines, writes `runs/training_pipeline.json`, and recommends whether to promote; `--promote` is required before it can overwrite `game_model.pkl`; `--debug-ai` now explains the model's action with confidence, danger, ore pull, and nearest-rock labels; `policy:ore-hunter` provides a reward-seeking baseline that spends health for collectable ore when lives allow.
- Verification: ran `python3 -m unittest test_train_pipeline.py test_policies.py test_compare_models.py test_model_report.py test_collect_policy_data.py test_play_with_model.py test_game_core.py`; ran `python3 -m unittest`; ran `python3 -X pycache_prefix=/private/tmp/rockfall-pycache -m py_compile collect_policy_data.py compare_models.py data_quality.py data_store.py difficulty.py evaluate_model.py features.py game.py game_audio.py game_core.py game_events.py inspect_data.py model_report.py play_with_model.py policies.py release_check.py run_model_experiment.py scores.py settings.py spawning.py train_model.py train_pipeline.py test_collect_policy_data.py test_compare_models.py test_data_quality.py test_data_store.py test_difficulty.py test_evaluate_model.py test_features.py test_game.py test_game_audio.py test_game_core.py test_inspect_data.py test_model_report.py test_play_with_model.py test_policies.py test_release_check.py test_run_model_experiment.py test_scores.py test_spawning.py test_train_model.py test_train_pipeline.py`; ran `python3 collect_policy_data.py --data /private/tmp/rockfall-v084-pipeline-policy.json --policy ore-hunter --games 2 --max-frames 300 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --report /private/tmp/rockfall-v084-pipeline-policy-report.json`; ran `python3 train_pipeline.py --data /private/tmp/rockfall-v084-pipeline-policy.json --candidate /private/tmp/rockfall-v084-pipeline-candidate.pkl --report /private/tmp/rockfall-v084-training-pipeline.json --games 1 --max-frames 240 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --estimators 5`; ran `python3 compare_models.py game_model.pkl --include-rule-baseline --include-ore-hunter-baseline --games 1 --max-frames 240 --difficulty normal --player-speed 8 --lives 3 --variant-profile variant-rich --report /private/tmp/rockfall-v084-comparison.json`; ran `python3 release_check.py --games 1 --max-frames 300 --difficulty normal --player-speed 8 --lives 3 --report /private/tmp/rockfall-v084-release-check.json`; ran `python3 model_report.py --games 1 --max-frames 240 --difficulty normal --player-speed 8 --lives 3 --report /private/tmp/rockfall-v084-model-report.json`; ran `python3 -m json.tool /private/tmp/rockfall-v084-pipeline-policy-report.json`; ran `python3 -m json.tool /private/tmp/rockfall-v084-training-pipeline.json`; ran `python3 -m json.tool /private/tmp/rockfall-v084-comparison.json`; ran `python3 -m json.tool /private/tmp/rockfall-v084-release-check.json`; ran `python3 -m json.tool /private/tmp/rockfall-v084-model-report.json`; ran a stale-text scan for old version/risk-bonus terms; ran `git diff --check`.
- Risks/Notes: the successful pipeline smoke used policy-generated ore-hunter data in `/private/tmp`, not human play data, and did not promote or replace `game_model.pkl`. Real promotion should wait for a larger fresh `ore_target_v2` human or mixed dataset and a longer evaluation run.
